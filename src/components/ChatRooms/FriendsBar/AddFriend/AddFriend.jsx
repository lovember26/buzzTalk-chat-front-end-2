...
import NoFriends from "components/ChatRooms/NoFriends";
...
export default function AddFriend() {
 ...
 console.log(Math.ceil(data.count/10));
  ...
  getUsers();
  const filteredNames = users.filter(user => user.username.toLowerCase().startsWith(query));
 ...
 // eslint-disable-next-line
  ...
  return <AddFriendWrapper>
    <AddFriendForm action="submit">
      <Search/>
      <input placeholder="You can add friends with their username" value={query} onChange={handleQuery}></input>
